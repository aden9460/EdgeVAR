            ...
            for names in sequential:
                ...
                for name in module_dict:
                    ...
                    if name == "ffn.fc2":
                        ...
                        model.blocks[i].attn.register_buffer('pruned_indices', torch.zeros(len(idx)).int())
                        ...
                        proj_qkv = (idx.unsqueeze(1) * 3 + torch.arange(3, device=idx.device)).reshape(-1)
                        proj_qkv = proj_qkv.tolist()
                        idx = idx.tolist()
...
def main(args):
    ...
    args.maxlayer = min(args.maxlayer, model.depth)
